...
class FilePath_View(Frame):
    ...
    def initUI(self, open_or_save_as='open', **kw):
        ...
        self.label_object.config(font=("Arial", 10, "bold"), width=30, anchor=E, justify="left")
        self.label_object.config(**kw)
        #Textbox:
        self.textbox = Entry(self)
        self.textbox.grid(row=0, column=1, sticky=W) 
        self.textbox.config(state=NORMAL, relief="ridge", font=("Arial", 10, "bold"), fg=fg, background=self.model.bgcolor, readonlybackground=self.model.bgcolor, width=self.file_textbox_width, justify="left")
