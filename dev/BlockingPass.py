...
class BlockingPass_Model():
    ...
    def get_compare_method_from_abbrev(self, abbrev):
        ...
        for text in self.compare_methods:
            if str(text[:3]).lower().strip() == abbrev:
                return_value = text
        return return_value
    ...
    def get_datacolumn_pos_from_datadict(self, column_name, mem_or_rec):
        ...
        print("In get_datacolumn_pos(), type of meta_values is: %s, SEEKING column_name=%s ... mem_or_rec=%s" % ( str(type(meta_values)), column_name, mem_or_rec) )
    ...
    def create_parmf_file_from_blocking_passes(self):
        ...
        with open(self.bigmatch_parmf_file_to_save, "w") as f: 
            ...
            if self.dedupe_single_file == True:
                ...
                dedupe_digit = "0"
            f.write(str(count_populated_blocking_passes) + " 1 1 0 1 " + str(dedupe_digit) + " 0 110 110 \n")       #self.pass_index +1
            ...
            for row in self.parmf_rows:
                ...
                if str(row["rowtype"]).lower() == "blocking_fields":
                    ...
                    blocking_row_string = str(row["recfile_column"]).lower().ljust(10) + bigspace + str(row["recfile_startpos"]).ljust(4) + " " + str(row["recfile_width"]).ljust(5) + " " + str(row["memfile_startpos"]).ljust(4) + " " + str(row["memfile_width"]).ljust(5) + " " + str(row["comparison_method"]).ljust(5) + " " + mvalue.ljust(7) + " " + uvalue.ljust(7)
    ...
    def get_datafield_name_from_dict_startpos(self, startpos, mem_or_rec):
        ...
        print("\n In get_datafield_name_from_dict_startpos(), mem_or_rec=%s, startpos=%s, stposindx=%s, result=%s" % (mem_or_rec, startpos, stposindx, return_value) )
        return return_value
            ...
            for parm in self.parmfileobj.parms:
                ...
                if str(control_type).lower()[:7] == "cutoff_" or str(control_type).lower()[:9] == "prcutoff_" or control_type == "uniqid":    #We can't use the First_Parm_In_Row for cutoff fields because they don't appear in a row with a Recfile_FIeld, the way BlockingField and MatchingField rows do.
                    ...
                    if str(parm["blocking_pass"]).lower() == blocking_pass and str(parm["row_type"]).lower() == row_type and str(parm["parm_type"]).lower() == control_type and str(parm["first_parm_in_row"]).lower() == data_column_name:
                        match = True
                if match:
                    print("\n\n *******FOUND A ROW WITH blocking pass %s, row_type %s, parm_type %s and first_parm_in_row %s" % (blocking_pass, row_type, control_type, data_column_name) )
                    print("PARMFILE PARM: blkpass: %s, row_index: %s, row_type: %s, num_parms in row: %s, parm_index: %s, parm_type: %s, parm_value: %s, first_parm_in_row: %s" % (parm["blocking_pass"], parm["row_index"], parm["row_type"], parm["num_parms_in_row"], parm["parm_index"], parm["parm_type"], parm["parm_value"], parm["first_parm_in_row"] ) )
...
class BlockingPass_View(Frame):
    ...
    def initUI(self, **kw):
        ...
        for item in self.model.meta_values_recfile:
            ...
            #opt = self.create_optmenu(fieldnames, curvalue, "opt" + self.rowtype[:5] + "_" + data_column_name + "_" + str(self.pass_index), vert_position, gridcolumn=1, width=12, rowindex=self.row_index, data_column_name=data_column_name, text=data_column_name)
