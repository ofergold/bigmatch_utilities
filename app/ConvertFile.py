...
class ConvertFile_Model():
    ...
    output_file = None			#Data dictionary file name and path, to which the metadata will be saved if user requests.
    ...
    def update_filepath(self, file_name_with_path='', callback_string='', alias=''):
        ...
        print("Master ConvertFile_Model object has gotten the alert: filename is %s and callback_string is '%s'" % (file_name_with_path, callback_string))
        if callback_string.lower().strip()[:4] == "load" or callback_string.lower().strip()[:4] == "open":
            ...
            #Refresh the view when the user selects a new file.
            if self.source_file: 
                self.display_view()
