...
class SAS7BDAT(object):
    ...
    def readVal(self, fmt, h, start, size):
        ...
        if fmt == 's':
            val = val.strip(b'\x00')
        #print("\n GMS in function readVal(), val=%s" % (val) )
        return val
    ...
    def _readHeader(self):
        ...
        with open(self.path, 'rb') as f:
            ...
            #GMS readVal() is returning a value of ZERO on this function call, so hl=0, which generates an error.
            ...
            print("\n GMS h: %s" % (h) )
            print("\n GMS hl: %s" % (hl) )
            #GMS TRY EXPLICITLY SETTING hl TO 288
            hl = 288    #GMS
            #GMS THIS IS DERAILING THE PROGRAM. HEADER HAS SIZE 288. THE PROGRAM EXPECTS A REMAINDER (MORE HEADER MATERIAL) BUT SEE WHAT HAPPENS IF WE SKIP THIS CHECK.
            '''h += f.read(hl - 288)
            if len(h) != hl:
                self.logger.error('[%s] header too short (not a sas7bdat '
                                  'file?)', os.path.basename(self.path))
            ...
            print("GMS pagesize: %s, pagecount: %s, datecreated: %s, sasrelease: %s, sashost: %s, osmaker: %s, osname: %s, osversion: %s" % (pagesize, pagecount, datecreated, sasrelease, sashost, osmaker, osname, osversion) )
