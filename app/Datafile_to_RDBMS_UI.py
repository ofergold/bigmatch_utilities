...
class Datafile_to_RDBMS_UI_View(Frame):
    ...
    def __init__(self, container, model):
        ...
        #Display the frame:
        print("\n" + "In Datafile_to_RDBMS_UI_View._init_, source_file = '" + str(self.model.source_file) + "' -- and show_view=" + str(self.model.show_view) )
    ...
    def initUI(self, **kw):
        ...
        if str(type(self.container)).lower().find(".tk") == -1:							#For testing, we might display this object directly in the Tkinter main window.  If this is the case, then don't call get_widget_position().
            stackslot = self.container.get_widget_position(self, "Datafile_to_RDBMS_UI_View.initUI()")
        else:
            stackslot = 0		
    ...
    '''def display_datadict_in_grid(self, **kw):
        ...
        #self.model.logobject.logit("\n Already_init=%s " % (str(already_init) ), True, True )   #... About to call clear_grid() from initUI() in Datafile_to_RDBMS_UI_View.
        print("\n Already_init=%s " % (str(already_init) ))   #... About to call clear_grid() from initUI() in Datafile_to_RDBMS_UI_View.
        ...
        if already_init == True:
            #self.model.logobject.logit("\n" + "About to call repopulate_grid() from Datafile_to_RDBMS_UI_View.display_datadict_in_grid(). # Columns: " + str(len(self.model.meta_columns)) +  "# Rownums: " + str(len(self.model.meta_rownums)) + " # meta-value-rows: " + str(len(self.model.meta_values)), True, True )
            print("\n" + "About to call repopulate_grid() from Datafile_to_RDBMS_UI_View.display_datadict_in_grid(). # Columns: " + str(len(self.model.meta_columns)) +  "# Rownums: " + str(len(self.model.meta_rownums)) + " # meta-value-rows: " + str(len(self.model.meta_values)))
            self.entry_grid.repopulate_grid(self.model.meta_columns, self.model.meta_rownums, self.model.meta_values)
        else:
            #self.model.logobject.logit("\n" + "About to call initialize_grid() from initUI() in Datafile_to_RDBMS_UI_View. # Columns: " + str(len(self.model.meta_columns)) +  "# Rows: " + str(len(self.model.meta_rownums)) + " # value-rows: " + str(len(self.model.meta_values)), True, True )
            print("\n" + "About to call initialize_grid() from initUI() in Datafile_to_RDBMS_UI_View. # Columns: " + str(len(self.model.meta_columns)) +  "# Rows: " + str(len(self.model.meta_rownums)) + " # value-rows: " + str(len(self.model.meta_values)))
    ...
    #***********************************************************************************************************************************
    def update_filepath(self, file_name_with_path='', callback_string='', alias=''):
        #self.model.logobject.logit("Master Datafile_to_RDBMS_UI_View object has gotten the alert: filename is %s and callback_string is %s" % (file_name_with_path, callback_string), True, True )
        print("Master Datafile_to_RDBMS_UI_View object has gotten the alert: filename is %s and callback_string is %s" % (file_name_with_path, callback_string))
