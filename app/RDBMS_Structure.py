...
class RDBMS_Structure_Model():
    ...
    row_index = 0                      #Counter for iterating thru the data file
    ...
    def __init__(self, parent_window, controller, db_platform=None, output_format=None, output_file=None, show_view=None, title='File converson', **kw):      #, bgcolor=gl_frame_color, frame_width=gl_frame_width, frame_height=gl_frame_height
        ...
        self.logobject.logit("\nIn RDBMS_Structure_Model._init_: db_platform='%s'output_format='%s' -- and type(controller)=%s" % (db_platform, output_format, type(controller)), True, True )
        ...
        try:
            ...
            for row_as_list in self.sql_result_with_column_info: 
                ...
                for col in row_as_list:
                    print("   %s" % ( col ) )                 #COL is a list containikng the column number and the column value
    ...
    def write_data_to_flat_file(self):
        ...
        if not self.output_file:
            ...
            print("\n\nERROR: %s" % (self.error_message) )
            return
        self.logobject.logit("\n In RDBMS_Structure_Model.write_data_to_flat_file()", True, True )
        self.row_index = 0				
        curvalue = ""
    ...
    def open_db_connection(self):
        try:
            ...
            self.error_message = "Error while opening database connection for %s catalog %s" % (self.db_platform, self.db_catalog) + "  " + sys.exc_info()[0]
            print("\n\nERROR: %s" % (self.error_message) )
    ...
    def instantiate_view_object(self, container):
        self.view_object = RDBMS_Structure_View(container, self) 
    ...
    def display_view(self, container=None):
        ...
        if self.view_object is None:                                #We have not yet instantiated the View.  Make sure to display any Open File dialogs at top before rendering other views.
            ...
            #print("\n In RDBMS_Structure_Model.display_view(), about to call view_object.initUI().")
    ...
    def display_user_buttons(self, container):
        ...
        self.btnSaveToFile = Button(self.button_frame, text="Display data", width=30, command=self.display_data)   #convert_txt_to_sqlite
        ...
        lbl.config(width=20, background=self.bgcolor, borderwidth=0, padx=3, pady=3)
        ...
        self.sql_entry_box.grid(row=0, column=3, sticky=W)
        self.sql_entry_box.bind(sequence="<FocusOut>", func=self.sqltext_handler)
        self.sql_entry_box.bind(sequence="<FocusIn>", func=self.sqltext_handler)
    ...
    def sqltext_handler(self, parm=None):		
    ...
    def update_filepath(self, file_name_with_path='', callback_string='', alias=''):
        ...
        print("Master RDBMS_Structure_Model object has gotten the alert: filename is %s and callback_string is '%s'" % (file_name_with_path, callback_string))
...
#******************************************************************************************
class RDBMS_Structure_View(Frame):
    debug = True
    container = None
    #controller = None
    model = None
    ...
    def __init__(self, container, model):
        ...
        #Display the frame:
        print("\n" + "In RDBMS_Structure_View._init_, db_catalog = '" + str(self.model.db_catalog) + "' -- and show_view=" + str(self.model.show_view) )
    ...
    def initUI(self, **kw):
        ...
        if str(type(self.container)).lower().find(".tk") == -1:							#For testing, we might display this object directly in the Tkinter main window.  If this is the case, then don't call get_widget_position().
            stackslot = self.container.get_widget_position(self, "RDBMS_Structure_View.initUI()")
        else:
            stackslot = 0		
    ...
    def display_data_simple(self, start_row=None, **kw):
        ...
        if self.start_row is None:
            self.start_row = 0
        self.model.logobject.logit("\n In RDBMS_Structure_View.display_data_simple(), start_row=%s and rows_to_display is %s" % (self.start_row, self.rows_to_display), True, True )
        self.row_index = 0				
        curvalue = 0 
        data_column_name = ""
    ...
    #***********************************************************************************************************************************
    def update_filepath(self, file_name_with_path='', callback_string='', alias=''):
        #self.model.logobject.logit("Master RDBMS_Structure_View object has gotten the alert: filename is %s and callback_string is %s" % (file_name_with_path, callback_string), True, True )
        print("Master RDBMS_Structure_View object has gotten the alert: filename is %s and callback_string is %s" % (file_name_with_path, callback_string))
